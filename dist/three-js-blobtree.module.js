        ...
        const processBranch = (root, next) => {
            ...
            if (!processed[curr.getKey()]) {
                if (curr_size === 1) {
                    ...
                    const nexts = [];
                    curr.getNeighbors().forEach((value) => {
...
const ImageSkeletonizer = {
    ...
    skeletonize: function (img_data, angle, weight_factor) {
     ...
     */
    skeletonizeQ: function (img_data) {
        const binary_img = new BinaryImage(img_data);
        const dist_img = new IntDistanceImage(3, 4, binary_img, 0);
     ...
     */
    drawHierarchyInImageData: function (h, img_data, mode) {
        const res = img_data;
     ...
     */
    drawSkeletonInCanvas: function (skel, cvs) {

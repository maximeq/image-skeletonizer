...
var l = s.getNeighbors().keys();
(y = s),
(s = s.getNeighbors().get(l.next().value)) === t &&
(s = y.getNeighbors().get(l.next().value));
c < 3 && ((r.x += s.getPosition().x), (r.y += s.getPosition().y)),
2 === c &&
((r.x = (r.x - 3 * t.getPosition().x) / 3),
(r.y = (r.y - 3 * t.getPosition().y) / 3)),
c++,
n.subPoints(s.getPosition(), t.getPosition());
var p = n.angle() - r.angle();
if ((Math.abs(p) < e || c < 3 || (d = !1), c >= 3)) {
var w = t.getWeight() / s.getWeight();
w < 1 && (w = 1 / w), w > i && (u = !1);
}
d && u && (t.removeNeighbor(y), s.removeNeighbor(y), t.addNeighbor(s)),
(a[y.getKey()] = !0),
(g = s.getNeighbors().size);
}
if (!a[s.getKey()])
if (1 === g)
(d && u) || (y && y.removeNeighbor(s)),
0 === c && t.removeNeighbor(s),
(a[s.getKey()] = !0);
else if (2 === g) h(y, s), (a[y.getKey()] = !0);
else {
var x = [];
s.getNeighbors().forEach(function(e, i, h) {
e !== y && e !== t && x.push(e);
}),
y && (t.removeNeighbor(y), s.removeNeighbor(y), t.addNeighbor(s)),
(a[s.getKey()] = !0);
for (var I = new Map(), m = 0; m < x.length; ++m) {
for (var v = m + 1; v < x.length; ++v) x[m].removeNeighbor(x[v]);
x[m].getNeighbors().forEach(function(t, e, i) {
I.set(e, t);
});
}
I.delete(s.getKey());
...
var s = t[o],
r = null;
s.getNeighbors().size > 1 &&
((r = new g(new n(s.getPosition().x, s.getPosition().y - 1), s.getWeight())),
s.addNeighbor(r),
(t[o] = r),
(s = r));
var a = {};

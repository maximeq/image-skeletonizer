...
export class SkeletonImage {
    ...
    thin() {
        ...
        for (let i = 1; i < this.height - 1; i++) {
            ...
            for (let j = 1; j < this.width - 1; j++) {
                index++;
                if ((this.data[index] & 1) === 0) continue;

                const voisins = this.getCurrentNeighborhood(index);

                if (
                    ((voisins & 7) === 0 && (voisins & 112) === 112) ||
     ...
     */
    skeletonizeEckhardtMaderlechner93(maxIter: number, distance_image: IntDistanceImage, threshold: number) {
        const size = this.width * this.height;

        const flag = new Array<number>(size); // interior (2) or boundary (1) pixel
        const flagSN = new Array<number>(size); // nb strong neighbors
        const flagN = new Array<number>(size); // nb neighbors
        const flagIBP = new Array<boolean>(size); // Inner Boundary Pixel
        const flagPIBP = new Array<boolean>(size); // Perfect Inner Boundary Pixel
        const flagSBP = new Array<boolean>(size); // Simple Boundary Pixel
        const flagEnd = new Array<number>(size); // Resulting image
        const flagDbg = new Array<number>(size); // Temporary image: color pixel

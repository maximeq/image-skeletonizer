...
export class IntDistanceImage {
    ...
    rebuild(c1: number, c2: number, source: BinaryImage, uncolored: number) {
        this.coeff = c1;
        this._buildDistanceImage(c1, c2, source, uncolored);
    }
    ...
    getXFromIndex(idx: number) {
        return idx % this.width;
    }
    getYFromIndex(idx: number) {
        return Math.floor(idx / this.width);
    }


    _buildDistanceImage(c1: number, c2: number, source: BinaryImage, uncolored: number) {

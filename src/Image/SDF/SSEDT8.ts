...
export class SSEDT8 {
    ...
    outside: number;
    outofrange: { x: number, y: number };
    inc: number;
    incMaxSize: number;

    constructor(grid: { x: number, y: number }[], width: number, height: number) {
        this.width = width;
        this.height = height;
        this.grid = grid;
        this.outside = 10000;
     ...
     */
    grid_get(grid: { x: number, y: number }[], x: number, y: number): { x: number, y: number } {
     ...
     */
    grid_put(grid: { x: number, y: number }[], x: number, y: number, p: { x: number, y: number }): void {
     ...
     */
    lengthSq = function (c: { x: number, y: number }): number {
     ...
     */
    grid_compare(g: { x: number, y: number }[], cell: { x: number, y: number }, x: number, y: number, offsetX: number, offsetY: number): { x: number, y: number } {
        const gOff = { x: 0, y: 0 };
        const get = this.grid_get(g, x + offsetX, y + offsetY);
     ...
     *   @param grid A grid
     */
    propagate(grid: { x: number, y: number }[]): void {
        let p: { x: number, y: number }, x: number, y: number;
     ...
     */
    signedDistanceFieldsFromGrid(): { data: Float32Array, width: number, height: number } {
        let x: number, y: number;
        const grid1: { x: number, y: number }[] = [];
        const grid2: { x: number, y: number }[] = [];
        let index;
        this.inc = 0;
        ...
        const distanceFields = new Float32Array(this.height * this.width);
        let dist1, dist2, dist;
     ...
     */
    unsignedDistanceFieldsFromGrid(): { data: Float32Array, width: number, height: number } {
        let x, y;
        const grid1: { x: number, y: number }[] = [];
        let index;
        this.inc = 0;

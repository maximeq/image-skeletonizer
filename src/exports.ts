...
const skeletonize = function (img_data: ImageData, angle?: number, weight_factor?: number) {
 ...
 */
const skeletonizeQ = function (img_data: ImageData) {
    const binary_img = new BinaryImage(img_data);
    const dist_img = new IntDistanceImage(3, 4, binary_img, 0);
 ...
 */
const drawHierarchyInImageData = function (h: SkeletonNode[], img_data: ImageData, mode: "circle" | "capsule"): ImageData {
    const res = img_data;
 ...
 */
const drawSkeletonInCanvas = function (skel: SkeletonNode[], cvs: HTMLCanvasElement) {
